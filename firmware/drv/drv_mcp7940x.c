...
#define MCP7940X_SLAVE_ADDRESS_CONTROL_BYTE 0x6F    // 01101111
#define MCP7940X_MAX_BYTE_ADDRESS           0x1F    // Este el la direccion del registro mayor
...
typedef struct
{
  t_MCP7940X_STATES sm_currentState;
  t_RW_PARAMS rwParams;
  t_MCP7940X_REGS_DATE sys;
  struct tm ts_local;
  DRV_MCP7940X_CALL_BACK cb;
} t_MCP7940X_DRV_DATA;

typedef struct
{
  t_MCP7940X_STATES stateOnSuccessful;
  t_MCP7940X_STATES stateOnError;
  void (*subTask)(void*);
  void *context;
  uint32_t state;
} t_SUB_TASK_DATA;
...
static t_MCP7940X_DRV_DATA drvData;
...
/* ************************************************************************** */
static void init_cb (t_MCP7940X_CALL_BACK_CONTEXT *ctx)
{
  _l ("(DRV_MCP7940) Initialized\n");
  drvData.cb = NULL;
}
...
static void i2c_task (void)
{
  switch (i2cData.state)
    {
    case BUSI2C_STATE_IDLE:
      break;
    case BUSI2C_STATE_PRE_INIT:
      break;
      ...
      if (!I2C5_IsBusy ())
        {
          switch (I2C5_ErrorGet ())
            {
            case I2C_ERROR_NONE:
              i2cData.state = BUSI2C_STATE_COMPLETED;
              break;
            case I2C_ERROR_BUS_COLLISION:
              i2cData.state = BUSI2C_STATE_COLLISION;
              break;
            case I2C_ERROR_NACK:
              i2cData.state = BUSI2C_STATE_NAK;
              break;
            default:
              i2cData.state = BUSI2C_STATE_ERROR;
              break;
    ...
    case BUSI2C_STATE_COLLISION:
      break;
    case BUSI2C_STATE_NAK:
      break;
    case BUSI2C_STATE_COMPLETED:
      break;
    case BUSI2C_STATE_ERROR:
      break;
    }
}
...
{
  if (i2cData.state != BUSI2C_STATE_IDLE)
    return 0; //Bus busy
  ...
  i2cData.state = BUSI2C_STATE_INIT; //Init I2C transaction
...
static void sub_task_init (void (*stf)(void*), void *subtaskContext, t_MCP7940X_STATES stateOK, t_MCP7940X_STATES stateError)
{
  SUB_TASK_STATE_INIT ();
...
static void sub_task_end (uint8_t end_type)
{
  ...
  if (end_type)
    ...
    {
      _l ("(DRV_MCP7940) Subtask end on error!\n");
...
static void sub_task_read_registers (void *ctx)
{
  ...
  switch (subtaskData.state)
    ...
    case ST_STATE_INIT__0__:
      ...
      if (i2c_rw (&sa, 1, ((t_RW_PARAMS*) ctx)->data, ((t_RW_PARAMS*) ctx)->length, MCP7940X_SLAVE_ADDRESS_CONTROL_BYTE))
        {
          SUB_TASK_STATE_NEXT ();
        }
      else
...
{
  static uint8_t sa[MCP7940X_MAX_BYTE_ADDRESS + 1];
  ...
  switch (subtaskData.state)
    ...
    case ST_STATE_INIT__0__:
      ...
      if (i2c_rw (sa, ((t_RW_PARAMS*) ctx)->length + 1, NULL, 0, MCP7940X_SLAVE_ADDRESS_CONTROL_BYTE))
        {
          SUB_TASK_STATE_NEXT ();
        }
      else
...
{
  memset (&drvData, 0, sizeof (t_MCP7940X_DRV_DATA));
  subtaskData.subTask = NULL;
  timeDelayms = 500;
  sub_task_init (sub_task_wait, &timeDelayms, MCP7940X_STATE_INIT, MCP7940X_STATE_ERROR);
...
{
  int yearAux;
  t_MCP7940X_CALL_BACK_CONTEXT ctx;
    ...
    case MCP7940X_STATE_INIT:
      {
        ...
        drvData.cb = init_cb;
        SET_DRV_STATE (MCP7940X_STATE_REFRESH_DATA);
      }
      break;
      /* IDLE */
    case MCP7940X_STATE_IDLE:
      break;
      /* REFRESH DATA */
    case MCP7940X_STATE_REFRESH_DATA:
      if (i2c_get_state () == BUSI2C_STATE_IDLE)
        {
          sub_task_init (sub_task_read_registers, &drvData.rwParams, MCP7940X_STATE_CHECK_ON, MCP7940X_STATE_ERROR);
        }
      break;
    case MCP7940X_STATE_CHECK_ON:
      //      _d ("(DRV_MCP7940) CHECK_ON\n sec00: %02X, min01: %02X, hour02: %02X, date04: %02X, month05: %02X, year06: %02X\n",
      //          (int) drvData.sys.seconds_00h,
      //          (int) drvData.sys.minutes_01h,
      //          (int) drvData.sys.hours_02h,
      //          (int) drvData.sys.date_04h,
      //          (int) drvData.sys.month_05h,
      //          (int) drvData.sys.year_06h);
      ...
      drvData.ts_local.tm_wday = drvData.sys.day - 1; //struct tm wday is 0-6 while rtc structure is 1-7 => (wday-1)
      drvData.ts_local.tm_mday = bcd2dec ((t_BCD_BYTE) (uint8_t) (drvData.sys.date_04h & 0x3F));
      drvData.ts_local.tm_mon = bcd2dec ((t_BCD_BYTE) (uint8_t) (drvData.sys.month_05h & 0x1F));
      drvData.ts_local.tm_mon -= 1; //struct tm mont is 0-11 while month is 1-12 => (mon - 1)
      ...
      if (drvData.sys.st && drvData.sys.vbaten)
        {
          ctx.res = MCP7940X_RES_SUCCESS;
          ctx.ts = mktime (&drvData.ts_local);
          ctx.reliable = true;
          _l ("(DRV_MCP7940) ST: %d, VBAT: %d. (Reliable) %s\n", drvData.sys.st, drvData.sys.vbaten, ctime (&ctx.ts));
          SET_DRV_STATE (MCP7940X_STATE_IDLE);
        }
      else
        {
          ctx.res = MCP7940X_RES_NOT_READY;
          ctx.ts = 0;
          ctx.reliable = false;
          _l ("(DRV_MCP7940) ST: %d, VBAT: %d. (NOT Reliable!)\n", drvData.sys.st, drvData.sys.vbaten);
          SET_DRV_STATE (MCP7940X_STATE_TURN_ON);
        }
      if (drvData.cb) drvData.cb (&ctx);
      break;
    case MCP7940X_STATE_TURN_ON:
      if (i2c_get_state () == BUSI2C_STATE_IDLE)
        {
          _l ("(DRV_MCP7940) Turn ON\n");
          drvData.rwParams.startAddress = REG_SECONDS;
          ...
          drvData.rwParams.data = drvData.sys.s;
          drvData.rwParams.length = 8; //Just update from 00h to 07h                  
          sub_task_init (sub_task_write_registers, &drvData.rwParams, MCP7940X_STATE_IDLE, MCP7940X_STATE_ERROR);
        }
      break;
      /* SET FULL DATE */
    case MCP7940X_STATE_REFRESH_FOR_UPDATE:
      if (i2c_get_state () == BUSI2C_STATE_IDLE)
        {
          sub_task_init (sub_task_read_registers, &drvData.rwParams, MCP7940X_STATE_UPDATING, MCP7940X_STATE_ERROR);
        }
      break;
    case MCP7940X_STATE_UPDATING:
      if (i2c_get_state () == BUSI2C_STATE_IDLE)
        {
          _l ("(DRV_MCP7940) Updating... %s\n", asctime (&(drvData.ts_local)));
          drvData.rwParams.startAddress = REG_SECONDS;
          ...
          drvData.sys.day = drvData.ts_local.tm_wday + 1; //struct tm wday is 0-6 while rtc structure is 1-7 => (wday + 1)
          drvData.sys.date_04h = (drvData.sys.date_04h & 0xC0) | dec2bcd (drvData.ts_local.tm_mday).b;
          drvData.sys.month_05h = (drvData.sys.month_05h & 0xC0) | dec2bcd (drvData.ts_local.tm_mon + 1).b; //struct tm mont is 0-11 while month_05 is 1-12 => (mon + 1)
          yearAux = drvData.ts_local.tm_year % 100;
          drvData.sys.year_06h = dec2bcd (yearAux).b;
          drvData.rwParams.data = drvData.sys.s;
          drvData.rwParams.length = 7; //Update register from 00h to 06h        
          //          _d ("(DRV_MCP7940) UPDATING\n sec00: %02X, min01: %02X, hour02: %02X, date04: %02X, month05: %02X, year06: %02X\n",
          //              (int) drvData.sys.seconds_00h,
          //              (int) drvData.sys.minutes_01h,
          //              (int) drvData.sys.hours_02h,
          //              (int) drvData.sys.date_04h,
      ...
      break;
      /* ERROR */
    case MCP7940X_STATE_ERROR:
      _l ("(DRV_MCP7940) ERROR! init task in 5 sec...\n");
      timeDelayms = 5000;
      sub_task_init (sub_task_wait, &timeDelayms, MCP7940X_STATE_INIT, MCP7940X_STATE_ERROR);
      break;
    default:
      _l ("(DRV_MCP7940) task state unknown!, ERROR!\n");
      SET_DRV_STATE (MCP7940X_STATE_ERROR);
      break;
    }
}
...
{
  if (drvData.sm_currentState != MCP7940X_STATE_IDLE)
    return -1;
  drvData.rwParams.startAddress = REG_SECONDS;
  drvData.rwParams.data = drvData.sys.s;
  drvData.rwParams.length = 9;
...
int32_t mcp7940x_set_ts (struct tm *ts)
{
  ...
  SET_DRV_STATE (MCP7940X_STATE_REFRESH_FOR_UPDATE);
